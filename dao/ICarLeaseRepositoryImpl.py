from Exception.myexceptions import CarNotFoundException, CustomerNotFoundException
...
class CarManagementImplementation(CarManagement):
    ...
    def addCar(self, car: Car):
        ...
        stmt = conn.cursor()
        query = "INSERT INTO vehicle_table(make, model, Year, dailyRate, status, passenger_capacity, engine_capacity) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        values = (carInfo['make'], carInfo['model'], carInfo['Year'], carInfo['dailyRate'], carInfo['status'],
                  carInfo['passenger_capacity'], carInfo['engine_capacity'])
